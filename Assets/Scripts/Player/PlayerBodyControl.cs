	...
	void FixedUpdate () {
		if (!playerLock) {
			...
			if (wallHug == 0f || Mathf.Sign(h) == wallHug)
				GetComponent<Rigidbody2D>().velocity = new Vector2 (moveVelocity * h, GetComponent<Rigidbody2D>().velocity.y);

			if(Input.GetButton("Jump") && jumpKeyDown && jumpKeyTimer < jumpKeyTime)
			{
				GetComponent<Rigidbody2D>().AddForce(-Physics2D.gravity.normalized * Mathf.Lerp (jumpForceLarge,jumpForceSmall,jumpKeyTimer/jumpKeyTime));
