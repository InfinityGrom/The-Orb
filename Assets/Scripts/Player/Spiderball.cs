...
public class Spiderball : MonoBehaviour {
	...
	void Start () {
		originalGravity = this.GetComponent<Rigidbody2D>().gravityScale;
	...
	void FixedUpdate () {
		...
		if (isConnected) {
			...
			if (h != 0 && !pbc.playerLock) {
				...
				if (currentSpeed < this.maxPlayerGeneratedSpeed) {
					this.GetComponent<Rigidbody2D>().AddForce(relativeRight.normalized * Mathf.Sign(h)  * spiderballMoveForce);
				}
			}
		}
		...
		if (Input.GetButton ("Jump") && isConnected
			...
			this.GetComponent<Rigidbody2D>().AddForce(lastCollision.contacts[0].normal.normalized * pbc.jumpForce);
