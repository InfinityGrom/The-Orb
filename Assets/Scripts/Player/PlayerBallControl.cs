...
public class PlayerBallControl : MonoBehaviour {
	...
	void OnCollisionEnter2D(Collision2D collision) {
		...
		foreach (ContactPoint2D contact in collision.contacts)
		{
			   ...
			   && dState == DeformationState.Normal)
			{
				...
				this.GetComponent<Rigidbody2D>().isKinematic = true;//Disable normal physics
	...
	private void ListenForJump() {
		    ...
		    && Time.timeSinceLevelLoad - springLastTime > Spring.springJumpTimeThreshold)
		{
			...
			//
			this.GetComponent<Rigidbody2D>().AddForce(-Physics2D.gravity.normalized * jumpForce);
			jumpTimer = 0.0f;
			hasContact = false;
	...
	void FixedUpdate () {
		...
		if(dState == DeformationState.Normal)
		{
			...
			if(h != 0)
			{
				...
				if (!spiderball && !balloonActive) {
					...
					{
						this.GetComponent<Rigidbody2D>().AddForce(relativeRight * h * moveForce * translationStoppingMultiplier);
					}
					else if (Mathf.Abs(this.GetComponent<Rigidbody2D>().velocity.x) < maxPlayerGeneratedSpeed)
					{
						this.GetComponent<Rigidbody2D>().AddForce(relativeRight * h * moveForce);
					}
				}
				//rotational movement
				if (Mathf.Sign (h) == Mathf.Sign (GetComponent<Rigidbody2D>().angularVelocity) && !hasContact)
				{
					this.GetComponent<Rigidbody2D>().AddTorque(-h * moveTorque * rotationStoppingMultiplier);
				}
				else if(Mathf.Abs (this.GetComponent<Rigidbody2D>().angularVelocity) < maxAngularVelocity && !hasContact)
				{
					this.GetComponent<Rigidbody2D>().AddTorque(-h * moveTorque);
				}
			}		
		}
		...
		{//Scale ball up along z axis of scale object at deformSpeed until it's scale has changed more than the intended change
			...
			if(deformTimer < deformTime)
			{
				...
				this.GetComponent<Rigidbody2D>().isKinematic = false;//Re-enabled rigidbody physics
				jumpLastTime = Time.timeSinceLevelLoad;
				...
				if(jumpBoosted)
				{
					...
					if(outVelocity.magnitude < (jumpForce/this.GetComponent<Rigidbody2D>().mass)*Time.fixedDeltaTime)
					{
						//Debug.Log("Back to original height");
						outVelocity.Normalize();
						outVelocity*=(jumpForce/this.GetComponent<Rigidbody2D>().mass)*Time.fixedDeltaTime;
					}
				}
				if(jumpDepressed)
				{
				...
				this.GetComponent<Rigidbody2D>().angularVelocity = outAngularVelocity;
				this.GetComponent<Rigidbody2D>().velocity = outVelocity;
		...
		prevVelocity = this.GetComponent<Rigidbody2D>().velocity;
		prevAngularVelocity = this.GetComponent<Rigidbody2D>().angularVelocity;
