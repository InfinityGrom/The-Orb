...
public class Ballonist : MonoBehaviour {
	...
	void Update () {
		...
		if (h != 0 /*&& (Mathf.Abs(balloonInst.rigidbody2D.velocity.x) < maxBalloonVelocity || Mathf.Sign(h) != Mathf.Sign(rigidbody2D.velocity.x))*/) {
			...
			if(Mathf.Sign(h) != Mathf.Sign (balloonInst.GetComponent<Rigidbody2D>().velocity.x))
			{
				balloonInst.GetComponent<Rigidbody2D>().AddForce(Vector2.right * h * moveForce * 1.5f);
			}
			else if (Mathf.Abs(balloonInst.GetComponent<Rigidbody2D>().velocity.x) < maxBalloonVelocity)
			{
				balloonInst.GetComponent<Rigidbody2D>().AddForce(Vector2.right * h * moveForce);
			}
		}
		...
		if ((balloonInst.transform.position - this.transform.position).magnitude > detachDistance) {
			...
			this.transform.GetComponent<Rigidbody2D>().AddForce(toBalloon * (jointForceConst + jointForceLin*distOut + jointForceQuad*distOut*distOut));
		}
		GetComponent<Rigidbody2D>().angularVelocity = 0f;

		if (balloonInst != null) {
			balloonInst.GetComponent<Rigidbody2D>().AddForce (new Vector2 (0f, balloonUpForce));
