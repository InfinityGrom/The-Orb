...
public class WaterBehavior : MonoBehaviour {
	...
	*/
	...
	void OnTriggerEnter2D(Collider2D c) {
		...
		//Handle noise
		Vector2 entryVelocity = c.gameObject.GetComponent<Rigidbody2D>().velocity;
	...
	void OnTriggerStay2D(Collider2D c) {
		...
		*/
		...
		Vector2 dampingForce = c.gameObject.GetComponent<Rigidbody2D>().velocity * dampingCoefficient;
		if (dampingForce.sqrMagnitude > maxDampingForce * maxDampingForce)
			dampingForce = dampingForce.normalized * maxDampingForce;

		c.gameObject.GetComponent<Rigidbody2D>().AddForce (buoyancyForce);
		c.gameObject.GetComponent<Rigidbody2D>().AddForce (dampingForce);
	}
