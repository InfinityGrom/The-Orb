	...
	void FixedUpdate () {
		if (dominantField == transform) {
			if (!directional) {
				...
				playerObj.GetComponent<Rigidbody2D>().AddForce(Physics2D.gravity.normalized * additionalForce / diff.sqrMagnitude);
			} else {
				playerObj.GetComponent<Rigidbody2D>().AddForce(direction.normalized * additionalForce);
			}
		}
	}
	...
	void OnTriggerEnter2D(Collider2D col)
	{
		if (col.tag == "Player") {
			...
			if (!directional) {
				...
				Physics2D.gravity = diff.normalized * gravityStrength;
				col.GetComponent<Rigidbody2D>().drag = appliedDrag;
			} else {
