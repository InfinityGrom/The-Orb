...
public class OneWay : MonoBehaviour {
		...
		//Has to check player velocity every update, else additional triggers would be necessary
	{
		...
		if (player.GetComponent<CircleCollider2D>() != null &&
		    	...
		    	transform.position.y + (GetComponent<BoxCollider2D>().offset.y + GetComponent<BoxCollider2D>().size.y / 2) * absScale.y - EPSILON) {
			    ...
			    player.transform.position.y + (player.GetComponent<BoxCollider2D>().offset.y - player.GetComponent<BoxCollider2D>().size.y / 2) * playerScale.y >= 
			    transform.position.y + (GetComponent<BoxCollider2D>().offset.y + GetComponent<BoxCollider2D>().size.y / 2) * absScale.y - EPSILON) {
				this.gameObject.layer = DEFAULT_LAYER;
				//switches to the default (collidable) layer
		} 
