...
public static class DialogueConstantParser
{
	...
	public static void ExecuteEvent(string eventName)
	{
		...
		case "BobSeparated":
			...
			if (obj != null) {
				...
				obj.GetComponent<Rigidbody2D>().AddForce(new Vector2(-3500f, 5000f));
			...
			if (obj != null) {
				...
				//inject sound
				obj.transform.GetChild(2).GetComponent<AudioSource>().Play();
			}
			//move camera out
			obj = GameObject.FindGameObjectWithTag("MainCamera");
			if (obj != null) {
		...
		case "TalkToRob":
			if (!EvaluateConstant("BobBodyGone")) {
				obj = GameObject.FindGameObjectWithTag("Villain");
				obj.transform.GetComponent<Interactable>().StepConvo();
				obj.transform.GetChild(4).GetComponent<AudioSource>().Play ();
				obj = GameObject.FindGameObjectWithTag("ScreenFader");
				obj.GetComponent<ScreenFading>().musicObj.GetComponent<AudioSource>().Stop();
			}
			break;
	...
	static IEnumerator RobScenePan() {
		...
		obj.transform.GetChild (3).GetComponent<AudioSource>().Play ();
	...
	static IEnumerator BobTalksToKingAnimation() {
		...
		obj.GetComponent<PlayerBallControl>().playerLock = true;
		obj.GetComponent<Rigidbody2D>().AddForce (new Vector2 (-615f, 0f));
		obj.GetComponent<Rigidbody2D>().AddTorque (200f);
	...
	static IEnumerator AnimateTeleporter() {
		...
		particleObj.GetComponent<AudioSource>().Play ();
		for (int i = 0; i < 60; i++)
		{
			particleObj.GetComponent<ParticleSystem>().emissionRate += 2f;
			if (i <= 50)
				particleObj.GetComponent<AudioSource>().pitch += 0.05f;
			yield return new WaitForSeconds(0.05f);
		}
		particleObj.GetComponent<ParticleSystem>().emissionRate = 0f;
		Transform particleObj2 = obj.transform.GetChild (2);
		particleObj2.gameObject.SetActive (true);
		particleObj.GetComponent<AudioSource>().Stop ();
		obj.GetComponent<AudioSource>().Play();
